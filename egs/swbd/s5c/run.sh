...
## Ongoing modificiations to add in CallHome Data
...
SWBDIR=/group/corporapublic/switchboard/switchboard1/
PRONLEX=
...
if [ $stage -le 0 ]; then
  echo "SWB DOWNLOAD"
  local/swbd1_data_download.sh $SWBDIR    #/disk/dataexport/corpora3/LDC/LDC97S62
...
if [ $stage -le 1 ]; then
  ...
  ## We basically need to use pronlex 
fi
...
#  local/swbd1_data_prep.sh $SWBDIR 
...
#  fisher_dirs="/group/corporapublic/fisher/english/transcripts/part1/ /group/corporapublic/fisher/english/transcripts/part2/"
...
#  local/eval2000_data_prep.sh /group/corporapublic/hub5/2000 /group/corporapublic/hub5/2000/transcr
#fi
#
#if [ $stage -le 6 ]; then
#  # prepare the rt03 data.  Note: this isn't 100% necessary for this
#  # recipe, not all parts actually test using rt03.
#  local/rt03_data_prep.sh /group/corporapublic/rich_transcription/rt-03/eval/speech/
#fi
#
#
#if [ $stage -le 7 ]; then
...
## for x in 1 2 3a 3b 4a; do grep 'Percent Total Error' exp/tri$x/decode_eval2000_nosp_sw1_tg/score_*/eval2000.ctm.filt.dtl | sort -k5 -g | head -1; done
